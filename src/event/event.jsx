...
export function Event() {
    ...
    function handleSkip() {
        if (queue.length) {
            ...
            setActivity((msgs) =>
                [
                    { id: Date.now(), text: `Now playing "${next.title}"` },
                    ...msgs,
                ].slice(0, 10),
            );
        } else {
    ...
    function handleMarkPlayed() {
        setActivity((msgs) =>
            [
                {
                    id: Date.now(),
                    text: `Finished "${nowPlaying?.title || ""}"`,
                },
                ...msgs,
            ].slice(0, 10),
        );
    ...
    function handleFavorite() {
        if (nowPlaying) {
            setActivity((msgs) =>
                [
                    { id: Date.now(), text: `Favorited "${nowPlaying.title}"` },
                    ...msgs,
                ].slice(0, 10),
            );
        }
            ...
            <section>
                ...
                <div id="currentTrack">
                    ...
                    <div>
                        ...
                        <div>
                            <progress
                                value={nowPlaying ? progress : 0}
                                max={nowPlaying ? nowPlaying.duration : 100}
                            ></progress>
                            <span>
                                {nowPlaying
                                    ? `${Math.floor(progress / 60)}:${String(progress % 60).padStart(2, "0")} / ${Math.floor(nowPlaying.duration / 60)}:${String(nowPlaying.duration % 60).padStart(2, "0")}`
                                    : "0:00 / 0:00"}
                            </span>
                        </div>
                        <p>
                            {nowPlaying ? `Requested by: You | Votes: —` : ""}
                        </p>
                    </div>
                </div>
            ...
            <section>
                ...
                <div>
                    ...
                    <div>
                        ...
                        <p>
                            Music Services:{" "}
                            <span id="musicStatus">Connected</span>
                        </p>
                        <p>
